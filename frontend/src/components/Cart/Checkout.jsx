...
const Checkout = () => {
  ...
  //Ensure crt is loaded before proceeding
  useEffect(() => {
    if(!cart || !cart.cart || cart.cart.products.length === 0) {
      navigate('/');
    }
  }, [cart, cart.cart, navigate]);
  ...
  const handleCreateCheckout = async (e) => {
    ...
    if(cart && cart.cart && cart.cart.products.length > 0) {
      const res = await dispatch(
        createCheckout({
          checkoutItems : cart.cart.products,
      ...
      if(res.payload && res.payload._id) {
        setCheckoutId(res.payload._id); //setting checkout id iff successfull
      }
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tighter">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="flex justify-between items-center text-lg mb-4">
          <p>Subtotal</p>
          <p>${cart?.cart?.totalPrice?.toLocaleString()}</p>
        </div>
